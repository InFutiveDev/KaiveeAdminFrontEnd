...
const Autocomplete = props => {
  ...
  const onSuggestionItemClick = (url, e) => {
    ...
    getSelectedOption(filteredData[activeSuggestion])
  ...
  const renderUngroupedSuggestions = () => {
    const { filterKey, suggestions, customRender, suggestionLimit, icon = '' } = props
  ...
  return (
    <div className='autocomplete-container' style={{ height: '38px' }} ref={container}>
      <Input
        type='text'
        onChange={e => {
          onChange(e)
          if (props.onChange) {
            props.onChange(e, filteredData[activeSuggestion])
          }
        }}
        autoComplete="off"
        name={props?.name || ''}
