...
import { GET_ALL_APPLIEDJOBS , GET_ALL_APPLIEDJOBS_EXPORT } from "../../../../redux/actions/appliedJob";
...
const CustomHeader = ({
  ...
  const handleExport = () => {
    dispatch(
        GET_ALL_APPLIEDJOBS_EXPORT(1, searchTerm, startDate, endDate)
    ).then((res) => {
      if (res?.data?.ApplicationData) {
        setDownloadTest(res?.data?.ApplicationData);
        setTimeout(() => {
...
const SubscriberPageList = () => {
  ...
  const handlePagination = (page) => {
    dispatch(
        GET_ALL_APPLIEDJOBS(page.selected + 1, searchTerm, startDate, endDate)
    );
  ...
  const handleSubmitSearch = () => {
    setCurrentPage(1);
    dispatch(GET_ALL_APPLIEDJOBS(1, searchTerm, startDate, endDate));
  };
  ...
  useEffect(() => {
    dispatch(GET_ALL_APPLIEDJOBS(1, searchTerm, "", ""));
  }, []);
