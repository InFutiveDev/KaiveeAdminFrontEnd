...
import { GET_ALL_HEBITS } from "../../../../redux/actions/habit";
// **  Components
...
const RiskList = () => {
  ...
  const handlePagination = (page) => {
    dispatch(GET_ALL_HEBITS(page.selected + 1, searchname));
  ...
  useEffect(() => {
    dispatch(GET_ALL_HEBITS(1, searchname));
  }, []);

  const CustomPagination = () => {
    const count = Number(Math.ceil(Habits?.pagination?.total / 10));
    return (
      <ReactPaginate
        previousLabel={""}
        nextLabel={""}
