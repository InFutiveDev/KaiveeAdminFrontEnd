    ...
    <div className='w-100'>
      <Row>
        ...
        <Col md='5' sm='5'>
          ...
          <Button.Ripple onClick={() => { history.push('/apps/media/add') }} color='primary'>
...
const Media = () => {
  ...
  const handlePagination = page => {
    dispatch(GET_ALL_MEDIA(page.selected + 1, searchname))
  ...
  useEffect(() => {
    dispatch(GET_ALL_MEDIA(1, searchname))
  }, [])

  const CustomPagination = () => {
    const count = Number(Math.ceil(media?.media?.pagination?.total / 10))
