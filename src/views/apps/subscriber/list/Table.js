...
import { GET_ALL_SUBSCRIBERS, GET_ALL_SUBSCRIBERS_EXPORT } from "../../../../redux/actions/subscriber";
...
const CustomHeader = ({
  ...
  const handleExport = () => {
    dispatch(
        GET_ALL_SUBSCRIBERS_EXPORT(1, searchTerm, startDate, endDate)
    ).then((res) => {
      if (res?.data?.pageData) {
        setDownloadTest(res?.data?.pageData);
        setTimeout(() => {
        ...
        <Col md="6" sm="6">
          <div>
            <Input
              id="search-user"
              className="w-100"
              placeholder="Search by Subsriber Email"
              value={searchTerm}
...
const SubscriberPageList = () => {
  // ** Store Vars
    ...
    const { data } = useSelector((state) => state.subscriber);
  ...
  const handlePagination = (page) => {
    dispatch(
        GET_ALL_SUBSCRIBERS(page.selected + 1, searchTerm, startDate, endDate)
    );
  ...
  const handleSubmitSearch = () => {
    setCurrentPage(1);
    dispatch(GET_ALL_SUBSCRIBERS(1, searchTerm, startDate, endDate));
  };
  ...
  useEffect(() => {
    dispatch(GET_ALL_SUBSCRIBERS(1, searchTerm, "", ""));
  }, []);
