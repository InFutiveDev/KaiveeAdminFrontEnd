    ...
    <div className='w-100'>
      <Row>
        ...
        <Col md='5' sm='5'>
          ...
          <Button.Ripple onClick={() => { history.push('/apps/category/add') }} color='primary'>
...
const CategoryList = () => {
  ...
  const handlePagination = page => {
    dispatch(GET_ALL_CATEGORIES(page.selected + 1, 10, searchname))
  ...
  const handleSearch = val => {
    ...
    const debounceTimer = setTimeout(() => {
      dispatch(GET_ALL_CATEGORIES(1, 10, val))
  ...
  useEffect(() => {
    dispatch(GET_ALL_CATEGORIES(1, 10, searchname))
  }, [])
  
  // ** Custom Pagination
  const CustomPagination = () => {
    const count = Number(Math.ceil(categories?.data?.pagination.total / 10))
    ...
    <Fragment>
      <Card>
        <DataTable
          ...
          data={categories?.data?.categoryData || []}
          subHeaderComponent={
