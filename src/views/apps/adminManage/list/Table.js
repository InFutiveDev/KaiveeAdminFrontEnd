...
import { GET_ADMIN_MANAGE } from "../../../../redux/actions/adminManage";
      ...
      <Row>
        ...
        <Col className="p-0" md="8" sm="4">
          <div className="d-flex justify-content-end">
            <Button.Ripple
              onClick={() => {
                history.push("/admin/apps/admin-management/add");
              }}
              color="primary"
            >
...
const ADMINMANAGELIST = () => {
  ...
  const history = useHistory()
  const { adminManageList } = useSelector((state) => state.adminManageReducer);
  // ** States
  ...
  const handlePagination = (page) => {
    dispatch(GET_ADMIN_MANAGE(page.selected + 1, searchname));
  ...
  const handleSearch = (val) => {
    ...
    const debounceTimer = setTimeout(() => {
      dispatch(GET_ADMIN_MANAGE(1, val))
  ...
  useEffect(() => {
    dispatch(GET_ADMIN_MANAGE(1, searchname));
  }, []);

  // ** Custom Pagination
  const CustomPagination = () => {
    const count = Number(
      Math.ceil(adminManageList?.data?.pagination?.total / 10)
    );
    ...
    <Fragment>
      <Card className="pr-1" style={{ borderRadius: "6px" }}>
        <DataTable
          ...
          data={adminManageList?.data?.data || []}
          subHeaderComponent={
