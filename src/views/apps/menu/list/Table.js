...
import { GET_ALL_MENU } from "../../../../redux/actions/menu";
      ...
      <Row>
        ...
        <Col className="p-0" md="8" sm="8">
          <div className="d-flex justify-content-end">
            <Button.Ripple
              onClick={() => {
                history.push("/admin/apps/menu/add");
              }}
              color="primary"
            >
...
const MenuTable = () => {
  ...
  const history = useHistory();
  const { menuAllList } = useSelector((state) => state.menuReducer);
  ...
  const handlePagination = (page) => {
    dispatch(GET_ALL_MENU(page.selected + 1, searchname));
  ...
  useEffect(() => {
    dispatch(GET_ALL_MENU(1, searchname));
  }, []);
