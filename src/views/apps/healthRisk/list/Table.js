    ...
    <div className='w-100'>
      <Row>
        <Col md='7' sm='8'>
          <div>
            <Input
              id='search-user'
              className='w-100'
              placeholder='Search Health Risk'
              value={searchTerm}
        ...
        <Col md='5' sm='5'>
          ...
          <Button.Ripple onClick={() => { history.push('/apps/risk/add') }} color='primary'>
...
const RiskList = () => {
  ...
  const store = useSelector(state => state.healthRisk)
  const { healthRisks } = store
  ...
  const handlePagination = page => {
    dispatch(GET_ALL_HEALTH_RISKS(page.selected + 1, searchname))
  ...
  const handleSearch = val => {
    ...
    const debounceTimer = setTimeout(() => {
      dispatch(GET_ALL_HEALTH_RISKS(1, val))
  ...
  useEffect(() => {
    dispatch(GET_ALL_HEALTH_RISKS(1, searchname))
  }, [])

  // ** Custom Pagination
  const CustomPagination = () => {
    const count = Number(Math.ceil(healthRisks?.data?.pagination.total / 10))
    ...
    <Fragment>
      <Card>
        <DataTable
          ...
          data={healthRisks?.data?.healthRiskData || []}
          subHeaderComponent={
