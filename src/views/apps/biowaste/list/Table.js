      ...
      <Row className={"justify-content-end"}>
        ...
        <Col md="5" sm="5">
          <div className="d-flex justify-content-end">
            <Button.Ripple
              onClick={() => {
                history.push("/apps/bio-waste/add");
              }}
              color="primary"
            >
...
const BioWasteList = () => {
  ...
  const handlePagination = (page) => {
    dispatch(GET_ALL_BIOWASTES(page.selected + 1, searchname));
  ...
  const handleSearch = (val) => {
    ...
    const debounceTimer = setTimeout(() => {
      dispatch(GET_ALL_BIOWASTES(1, val));
  ...
  useEffect(() => {
    dispatch(GET_ALL_BIOWASTES(1, searchname));
  }, []);

  const CustomPagination = () => {
    const count = Number(Math.ceil(bioWaste?.pagination?.total / 10));
