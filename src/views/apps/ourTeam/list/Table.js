...
import { GET_ALL_OURTEAMS } from "../../../../redux/actions/ourTeam";
      ...
      <Row>
        ...
        <Col md="5" sm="5">
          <div className="d-flex justify-content-end">
            <Button.Ripple
              onClick={() => {
                history.push("/apps/our-team/add");
              }}
              color="primary"
            >
...
const OurTeamList = () => {
  ...
  const handlePagination = (page) => {
    dispatch(GET_ALL_OURTEAMS(page.selected + 1, searchname));
  ...
  const handleSearch = (val) => {
    ...
    const debounceTimer = setTimeout(() => {
      dispatch(GET_ALL_OURTEAMS(1, val));
  ...
  useEffect(() => {
    dispatch(GET_ALL_OURTEAMS(1, searchname));
  }, []);
  console.log("outTeam--[[[[", ourTeam);
  const CustomPagination = () => {
    const count = Number(Math.ceil(ourTeam?.pagination?.total / 10));
